...
param_labels       = {'eta_к', 'eta_т', 'eta_в', 'eta_тв', 'eta_с2', 'eta_г'};
param_labels_latex = {'eta_к', 'eta_т', 'eta_в', 'eta_тв', 'eta_с2', 'eta_г'};
...
lb = [0.80,  0.80,  0.75,  0.85,  0.83, 0.95];   % 下界 / Нижняя граница
ub = [0.90,  0.99,  0.95,  0.97,  0.99, 0.999];  % 上界 / Верхняя граница

% “真值”（用于生成虚拟观测数据） / Истинные значения (для генерации виртуальных наблюдений)
theta_true = [0.86, 0.92, 0.86, 0.91, 0.93, 0.980];
...
%% 热力学查表：压气机/燃气焓 / Термодинамическая таблица: энтальпия CpT*
...
%% 热力学查表：燃料焓 / Термодинамическая таблица: энтальпия топлива iT*
...
%% 热力学数据表 / Таблица термодинамических данных
...
function [y, aux] = engine_forward(theta_s, theta_f, cond)
    ...
    eta_v  = theta_s(3);    % 风扇效率 / КПД вентилятора
    eta_tv = theta_s(4);    % 涡轮-风扇效率 / КПД турбины вентилятора
    eta_c2 = theta_s(5);    % 外涵喷管速度系数 / КПД сопла второго контура
    eta_g  = theta_s(6);    % 燃烧效率 / КПД камеры сгорания (eta_г)
    ...
    % 固定热力学常数 / Фиксированные термодинамические константы
    lambda = 1.03;      % 修正系数 / Поправочный коэффициент
    H_u    = 43000;     % 燃料低热值 / Низшая теплота сгорания топлива [кДж/кг]
    ...
    try
        ...
        % (5) 相对耗油量 — 燃烧室能量平衡 + 热力学查表插值
        %     g_T = (CpT_g - CpT_k) / (H_u * eta_g - iT_g + CpT_k)
        %     Уравнение баланса энергии камеры сгорания (таблица кДж/кг)
        CpT_k = interp_CpT(T_k);    % 压气机出口焓 / энтальпия за компрессором
        CpT_g = interp_CpT(T_g);    % 涡轮前燃气焓 / энтальпия газа перед турбиной
        iT_g  = interp_iT(T_g);     % 燃料焓 / энтальпия топлива при T_g
        denom_gT = H_u * eta_g - iT_g + CpT_k;
        if abs(denom_gT) < 1e-6, return; end
        g_T = (CpT_g - CpT_k) / denom_gT;
        if ~isfinite(g_T) || g_T <= 0, return; end

        % (6) 燃烧室前总压恢复系数 / коэффициент восстановления давления во входном устройстве
        sigma_bx = sigma_cc * sigma_kan;

        % (7) 单位自由能 удельная свободная энергия
        exp_T = (kT - 1) / kT;
        ...
        % (8) 最优自由能分配系数 Оптимальное значение коэффициента распределения
        ...
        % (9) 内外涵喷流速度与比推力 / Скорости струй внутреннего и внешнего контуров и удельная тяга
        ...
        % (10) 比油耗 / Удельный расход топлива
        denom_C = R_ud * (1 + m);
